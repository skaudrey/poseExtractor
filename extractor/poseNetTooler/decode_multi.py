from ..poseNetTooler.decode import *
from ..poseNetTooler.constants import *
import torch
...
def get_instance_score_fast(
    ...
    nk,nc = keypoint_coords.shape
    kps_coords = np.ndarray((nk+1,nc))
    kps_coords[:-1,:] = keypoint_coords
    kps_coords[-1,:] = (keypoint_coords[5]+keypoint_coords[6])/2.0

    kps_scores = np.ndarray((nk+1))
    kps_scores[:-1] = keypoint_scores
    kps_scores[-1] = (keypoint_scores[5]+keypoint_scores[6])/2.0

    if exist_pose_coords.shape[0]:
        s = np.sum((exist_pose_coords - kps_coords) ** 2, axis=2) > squared_nms_radius
        not_overlapped_scores = np.sum(kps_scores[np.all(s, axis=0)])
    else:
        not_overlapped_scores = np.sum(kps_scores)
    return not_overlapped_scores / len(kps_scores)
...
def build_part_with_score_torch(score_threshold, local_max_radius, scores):
    ...
    max_loc_idx = torch.nonzero(max_loc, as_tuple=False)
...
def decode_multiple_poses(
    ...
    for root_score, (root_id, root_coord_y, root_coord_x) in zip(part_scores, part_idx):
        ...
        if min_pose_score == 0. or pose_score >= min_pose_score:
            ...
            pose_keypoint_scores[pose_count, :-1] = keypoint_scores
            pose_keypoint_coords[pose_count, :-1, :] = keypoint_coords
            # set neck as the average of left shoulder and right shoulder, so is the confidence score.
            pose_keypoint_scores[pose_count, -1] = (keypoint_scores[5]+keypoint_scores[6])/2.0 # neck: the average of left shoulder and right shoulder
            pose_keypoint_coords[pose_count, -1, :] = (keypoint_coords[5]+keypoint_coords[6])/2.0
            pose_count += 1
