    ...
    def __getitem__(self, index):
        ...
        for frame_info in video_info['data']:
            ...
            for m, skeleton_info in enumerate(frame_info["skeleton"]):
                ...
                nc = pose.shape[-1]
                for inc in range(nc): # 2d or 3d
                    data_numpy[inc, frame_index, :, m] = pose[:,inc].reshape((-1))
                data_numpy[-1, frame_index, :, m] = np.array(score,dtype=np.float)
