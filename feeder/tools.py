...
def random_move(data_numpy,
    ...
    node = np.arange(0, T, T * 1.0 / move_time).round().astype(int) # (0,150,150), the interval is 150
    node = np.append(node, T)
    num_node = len(node)

    A = np.random.choice(angle_candidate, num_node) # randomly choose num_node item from angle_candidate
    ...
    for i in range(num_node - 1):
        a[node[i]:node[i + 1]] = np.linspace(
            A[i], A[i + 1], node[i + 1] - node[i]) * np.pi / 180 # angle
        s[node[i]:node[i + 1]] = np.linspace(S[i], S[i + 1],
                                             node[i + 1] - node[i]) # scale
        t_x[node[i]:node[i + 1]] = np.linspace(T_x[i], T_x[i + 1],
                                               node[i + 1] - node[i]) # translation_x
        t_y[node[i]:node[i + 1]] = np.linspace(T_y[i], T_y[i + 1],
                                               node[i + 1] - node[i]) # translation_y

    theta = np.array([[np.cos(a) * s, -np.sin(a) * s],
                      [np.sin(a) * s, np.cos(a) * s]]) # transformation matrix, each frame will have one

    # perform transformation
    for i_frame in range(T):
        xy = data_numpy[0:2, i_frame, :, :]
        new_xy = np.dot(theta[:, :, i_frame], xy.reshape(2, -1))
        new_xy[0] += t_x[i_frame] # every joint in the same frame will have same translation
